...
-- Reads TERMINAL_IDS, BUYIN, and STARTING_CHIPS from config.txt
-- Start tournament manually with 'start' command

-- Load configuration
local function loadConfig()
    local config = {TERMINAL_IDS = {391, 393}, BUYIN = 300, STARTING_CHIPS = 3000}
    if fs.exists("config.txt") then
        ...
        for line in content:gmatch("[^\r\n]+") do
            ...
            elseif key == "STARTING_CHIPS" then
                config.STARTING_CHIPS = tonumber(value) or 3000
            end
        end
    else
        ...
        file.writeLine("STARTING_CHIPS=3000")
        file.close()
    end
    return config
end
...
local STARTING_CHIPS = config.STARTING_CHIPS
local MAX_PLAYERS = 9
...
function main()
    ...
    while true do
        if game_state == "registration" then
            ...
            print("Type 'start' to begin the tournament (requires at least 2 players).")
            while game_state == "registration" do
                parallel.waitForAny(
                    function()
                        -- Handle rednet messages for registration
                        ...
                        if message.type == "register" and #players < MAX_PLAYERS then
                            ...
                            elseif message.name and message.name ~= "" then
                                if message.success then
                                    ...
                                    print(message.name .. " registered (" .. #players .. " players total)")
                                    saveState()
                                else
                    ...
                    function()
                        -- Handle terminal input for manual start
                        local input = io.read()
                        if input:lower() == "start" then
                            if #players >= 2 then
                                ...
                                print("Not enough players. Need at least 2 players to start.")
                                print("Current player count: " .. #players)
                            end
                        end
                    end
                )
