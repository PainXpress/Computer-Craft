...
local conversion_rate = 0.1 -- 1 currency = 0.1 chips
local state = "locked"
local message = ""
...
-- Handle rednet requests
function handleRednet()
    while true do
        local _, message = rednet.receive()
        if message.type == "deduct" then
            local balance, err = readBalance()
            if balance and balance >= message.amount then
                balance = balance - message.amount
                if writeBalance(balance) then
                    rednet.send(SERVER_ID, {type = "deduct_response", success = true, player = message.player})
                else
                    rednet.send(SERVER_ID, {type = "deduct_response", success = false, player = message.player, error = "Error writing to disk"})
                end
            else
                rednet.send(SERVER_ID, {type = "deduct_response", success = false, player = message.player, error = err or "Insufficient balance"})
            end
        end
    end
end
